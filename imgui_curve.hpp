...
namespace ImGui
{
	int Curve(const char *label, const ImVec2& size, ImVec4 *points, const size_t maxpoints );
	float CurveValue(float p, size_t maxpoints, const ImVec4 *points);
	float CurveValueSmooth(float p, size_t maxpoints, const ImVec4 *points);
};
...
namespace ImGui
{
	...
	void spline2( const float *key, size_t num, int dim, float t, float *v )
	{
		...
		for( int i=0; i<4; i++ )
		{
			int kn = k+i-2; if( kn<0 ) kn=0; else if( kn>int(num-1) ) kn=(int)num-1;
	...
	float CurveValueSmooth2(float p, size_t maxpoints, const ImVec4 *points)
	{
		...
		spline2( input, maxpoints, 1, p, output );

		delete [] input;
		return output[0];
	}

	float spline1( const ImVec4 *key, int num, float t )
	{
		if (num < 2 || key == 0) return 0.f;
		if (t < 0) return key[0].y;
		...
		// find key
		int k = 0; while( key[k].x < t && k < num ) k++;

		// interpolant
		const float h = (t-key[k-1].x)/(key[k].x-key[k-1].x);
	...
	float CurveValueSmooth(float p, size_t maxpoints, const ImVec4 *points)
	{
		return spline1( points, (int)maxpoints, p );
	}

	float CurveValue(float p, int maxpoints, const ImVec4 *points)
	{
		if (maxpoints < 2 || points == 0)
			return 0;
		if (p < 0) return points[0].y;
	...
	int Curve(const char *label, const ImVec2& size, ImVec4 *points, const size_t maxpoints )
	{
		...
		int maxp = 0;
		while (maxp < maxpoints && points[maxp].x >= 0) maxp++;
		...
		if (hovered)
		{
			...
			if (g.IO.MouseDown[0])
			{
				...
				int left = 0;
				while (left < maxp && points[left].x < pos.x) left++;
				if (left) left--;

				ImVec2 p;
				p.x = points[left].x - pos.x;
				p.y = points[left].y - pos.y;

				float p1d = sqrt(p.x*p.x + p.y*p.y);
				p.x = points[left+1].x - pos.x;
				p.y = points[left+1].x - pos.y;

				float p2d = sqrt(p.x*p.x + p.y*p.y);
				int sel = -1;
				if (p1d < (1 / 8.0)) sel = left;
				if (p2d < (1 / 8.0)) sel = left + 1;
