...
ImFontAtlas*      GImDefaultFontAtlas = 0;
...
void ImGui::CreateInstance()
{
    if (0 == GImDefaultFontAtlas)	GImDefaultFontAtlas = new ImFontAtlas();
    if (0 == GImGui) GImGui = CreateContext(); //new ImGuiContext();
}

void ImGui::DestroyInstance()
{
    if (GImDefaultFontAtlas)	{ delete GImDefaultFontAtlas; GImDefaultFontAtlas = 0; }
...
ImGuiStyle::ImGuiStyle()
{
    ...
    Colors[ImGuiCol_MaximizeButtonActive]   = ImVec4(0.20f, 0.70f, 0.70f, 1.00f);
...
{ 
    ...
    c.x *= GImGui->Style.Brightness;
    c.y *= GImGui->Style.Brightness;
    c.z *= GImGui->Style.Brightness;
...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImVec2 lineSize        = ImMax(window->DC.CurrentLineSize, size);
    ...
    if (!window->DC.CurrentLayout || window->DC.CurrentLayout->Type == ImGuiLayoutType_Vertical)
    {
        ...
        window->DC.CursorPos = ImVec2(
            ...
            (float)(int)(window->DC.CursorPos.y + lineSize.y + g.Style.ItemSpacing.y));
        window->DC.CursorMaxPos.x = ImMax(window->DC.CursorMaxPos.x, window->DC.CursorPosPrevLine.x);
        window->DC.CursorMaxPos.y = ImMax(window->DC.CursorMaxPos.y, window->DC.CursorPos.y);

        window->DC.PrevLineSize = ImVec2(window->DC.CurrentLineSize.x, lineSize.y);
    }
    else
    {
        // Move cursor to next horizontal line, x to right, y to top of last line.
        window->DC.CursorPos = ImVec2(
            (float)(int)(window->DC.CursorPos.x + lineSize.x + g.Style.ItemSpacing.x),
            (float)(int)(window->DC.CursorPosPrevLine.y));
        window->DC.CursorMaxPos.x = ImMax(window->DC.CursorMaxPos.x, window->DC.CursorPosPrevLine.x);
        window->DC.CursorMaxPos.y = ImMax(window->DC.CursorMaxPos.y, window->DC.CursorPos.y + lineSize.y + g.Style.ItemSpacing.y);
...
void ImGui::NewFrame()
{
    ...
    if (g.HoveredWindow && g.IO.MouseWheel != 0.0f && !g.HoveredWindow->Collapsed)
    {
        ...
        if (g.IO.KeyCtrl)
        {
            if (g.IO.FontAllowUserScaling && !(g.HoveredWindow->Flags & ImGuiWindowFlags_NoZoom) )
            {
                // Zoom / Scale window
...
void ImGui::RenderCollapseTriangle(ImVec2 p_min, bool is_open, float scale)
{
    ...
    ImVec2 center = p_min + ImVec2(h*0.50f, 0.f);
    ...
    window->DrawList->AddTriangleFilled(a, b, c, GetColorU32(ImGuiCol_CollapseTriangle));
}
...
static bool BeginPopupEx(const char* str_id, ImGuiWindowFlags extra_flags)
{
    ...
    ImGuiWindowFlags flags = extra_flags|ImGuiWindowFlags_Popup|ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoMove|ImGuiWindowFlags_NoSavedSettings;
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    const int titlebar_active = g.FocusedWindow && window->RootNonPopupWindow == g.FocusedWindow->RootNonPopupWindow;
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if ( !(flags & ImGuiWindowFlags_ChildWindow))
            {
                ...
                ImVec2 shadowSize[2];
                ImVec2 ofs[2] = { ImVec2(nudge,nudge+0.f),ImVec2(nudge,nudge) };
                ImU32 col[2];
                ImRect shadow;
                int shadowFlags;
                ...
                if (titlebar_active)
                {
                    ...
                    shadowSize[1].x = 48+nudge;
                    shadowSize[1].y = 70+nudge;
                    //shadowFlags &= ~2  & ~16; //top center, center center off
                    //ofs[0].y += 8.f;
                }
                else
                {
                    shadowSize[0].x = 32+nudge;
                    shadowSize[0].y = 24+nudge;
                    shadowSize[1].x = 32+nudge;
                    shadowSize[1].y = 36+nudge;
                }
                ...
                {
                    shadowSize[0].x = 14+nudge;
                    shadowSize[0].y = 4+nudge;
        ...
        window->DC.Indent = window->WindowPadding - window->Scroll;
        window->DC.GroupOffset = ImVec2(0.0f,0.0f);
        window->DC.ColumnsOffset = ImVec2(0.0f,0.0f);
        window->DC.CursorStartPos = window->Pos + window->DC.Indent + window->DC.ColumnsOffset + ImVec2(0.0f, window->TitleBarHeight() + window->MenuBarHeight());
...
{
    ImGuiWindow* window = GetCurrentWindow();
    float target_y = window->DC.CursorPosPrevLine.y + (window->DC.PrevLineSize.y * center_y_ratio) + (GImGui->Style.ItemSpacing.y * (center_y_ratio - 0.5f) * 2.0f); // Precisely aim above, in the middle or below the last line.
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        // Framed type
        RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
        RenderCollapseTriangle(bb.Min + ImVec2(0.0f, bb.GetHeight()*0.5f), is_open, style.CollapseTriangleScale);
        if (g.LogEnabled)
        {
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderCollapseTriangle(bb.Min + ImVec2(padding.x, bb.GetHeight()*0.5f), is_open, style.CollapseTriangleScale * 0.70f);
        if (g.LogEnabled)
...
bool ImGui::DragFloatN2(const char* label, float* v, int components, const float *v_speed, const float *v_min, const float* v_max, const char* display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    {
        PushID(i);
        value_changed |= DragFloat("##v", &v[i], v_speed[i], v_min[i], v_max[i], display_format, power);
        SameLine(0, g.Style.ItemInnerSpacing.x);
        PopID();
        PopItemWidth();
    }
...
// Combo box function.
bool ImGui::Combo(const char* label, int* current_item, ImGuiItemGetter items_getter, void* data, int items_count, int height_in_items, int columns)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderCollapseTriangle(ImVec2(frame_bb.Max.x-arrow_size, frame_bb.Min.y) + ImVec2( style.FramePadding.x,frame_bb.GetHeight()*0.5f), true,style.CollapseTriangleScale);
    ...
    if (IsPopupOpen(id))
    {
        ...
        if ( columns <= 1 )
        {
            ...
            const ImGuiWindowFlags flags = ImGuiWindowFlags_ComboBox | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize | ((window->Flags & ImGuiWindowFlags_ShowBorders) ? ImGuiWindowFlags_ShowBorders : 0);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    else
    {
        ...
        RenderCollapseTriangle(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f, 0.0f), false,style.CollapseTriangleScale);
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha)
{
    ...
    if (edit_mode == ImGuiColorEditMode_UserSelect || edit_mode == ImGuiColorEditMode_UserSelectShowButton)
        edit_mode = g.ColorEditModeStorage.GetInt(id, -1) % 4;

    if (edit_mode < 0) edit_mode = ImGuiColorEditMode_HSB;

    float f[4] = { col[0], col[1], col[2], col[3] };
    if (edit_mode == ImGuiColorEditMode_HSV || edit_mode == ImGuiColorEditMode_HSB)
    ...
    case ImGuiColorEditMode_HSB:
        {
            ...
            const char* fmt_table[4][4] =
            {
                {   "%3.0f",   "%3.0f",   "%3.0f",   "%3.0f" },
                { "R:%3.0f", "G:%3.0f", "B:%3.0f", "A:%3.0f" },
                { "H:%3.0f", "S:%3.0f", "V:%3.0f", "A:%3.0f" },
                { "H:%3.1f\xc2\xb0", "S:%2.1f%%", "B:%2.1f%%", "A:%2.1f%%" }
            };
            const float mult[4][4] =
            {
                { 1.f, 1.f, 1.f, 1.f },
                { 1.f, 1.f, 1.f, 1.f },
                { 1.f, 1.f, 1.f, 1.f },
                { 360.f, 100.f, 100.f, 100.f },
            };

            const char** fmt = fmt_table[0];
            if (hide_prefix) fmt = fmt_table[0];
            else if (edit_mode == ImGuiColorEditMode_HSB) fmt = fmt_table[3];
            else if (edit_mode == ImGuiColorEditMode_HSV) fmt = fmt_table[2];
            else if (edit_mode == ImGuiColorEditMode_RGB) fmt = fmt_table[1];
            ...
            for (int n = 0; n < components; n++)
            {
                ...
                if (edit_mode == ImGuiColorEditMode_HSB)
                {
                    f[n] *= mult[edit_mode][n];
                    value_changed |= DragFloat(ids[n], &f[n], 0.125f, 0, mult[edit_mode][n], fmt[n]);
                    f[n] /= mult[edit_mode][n];
                    i[n] = IM_F32_TO_INT8_UNBOUND(f[n]);
                }
                else
                {
    ...
    {
        SameLine(0, style.ItemInnerSpacing.x);
        const char* button_titles[4] = { "RGB", "HSB", "HSV", "HEX" };
    ...
    for (int n = 0; n < 4; n++)
        f[n] = i[n] / 255.0f;
    if (edit_mode == 1 || edit_mode == ImGuiColorEditMode_HSB)
...
void ImGui::BeginGroup()
{
    ...
    window->DC.GroupOffset = window->DC.CursorPos - window->Pos - window->DC.ColumnsOffset;
...
void ImGui::EndGroup()
{
    ...
    window->DC.GroupOffset = window->DC.Indent;
    window->DC.LogLinePosY = window->DC.CursorPos.y - 9999.0f;
...
static void BeginLayout(ImGuiID id, ImGuiLayoutType type, ImVec2 size, float align)
{
    ...
    // Start capture
    layout->NextItemIndex = 0;
    layout->StartPos      = ImGui::GetCursorScreenPos();
    layout->MaxExtent     = 0.0f;
...
static void DistributeAvailableLayoutSpace(ImGuiLayout* layout)
{
    ...
    float error          = 0.0f;
    ImGuiLayoutItem* last_span = NULL;
    for (int i = 0; i < layout->Items.size(); ++i)
    {
        ImGuiLayoutItem& item = layout->Items[i];
        if (item.Type != ImGuiLayoutItemType_Spring)
        ...
        if (free_space > 0.0f && total_spring_weight > 0.0f)
        {
            ...
            float span_end    = free_space * next_weight / total_spring_weight;

            item.SpringSize = (span_end - span_start) + item.SpringSpacing;
            error          += fmodf(item.SpringSize, 1.0f);
            item.SpringSize = floorf(item.SpringSize);
            if (error >= 1.0f)
            {
                item.SpringSize += 1.0f;
                error           -= 1.0f;
            }
            ...
            item.SpringSize = item.SpringSpacing;
    }
...
static ImGuiLayoutItem* PushNextLayoutItem(ImGuiLayout* layout, ImGuiLayoutItemType type)
{
    IM_ASSERT(layout->NextItemIndex <= layout->Items.size());
    ...
    if (layout->NextItemIndex < layout->Items.size())
    {
        ...
        if (item.Type != type)
        {
            ...
            layout->Dirty |= ImGuiLayoutDirtyFlags_ItemReplaced;
        }
    }
    else
    {
    ...
    return &layout->Items[layout->NextItemIndex];
}

static void BeginLayoutItem(ImGuiLayout* layout)
{
    PushNextLayoutItem(layout, ImGuiLayoutItemType_Item);
    ...
    if (layout->Type == ImGuiLayoutType_Horizontal)
    {
        ...
        if (layout->Bounds.y > item.Size.y)
        {
            ...
            ImGui::SetCursorPosY(new_position);
        }
    }
    else
    {
        ...
        if (layout->Bounds.x > item.Size.x)
        {
            ...
            ImGui::SetCursorPosX(new_position);
        }
    }
...
static void EndLayoutItem(ImGuiLayout* layout, bool is_closing)
{
    IM_ASSERT(layout->NextItemIndex < layout->Items.size());
    ...
    ImVec2 new_item_size = ImGui::GetItemRectSize();
    ...
    if (layout->Type == ImGuiLayoutType_Horizontal)
    {
        window->DC.CursorPos.x = window->DC.CursorMaxPos.x;
        if (!is_closing)
            window->DC.CursorMaxPos.y = window->DC.CursorPos.y;
    }
    else
    {
        float padding = window->DC.CursorPos.y - (window->DC.CursorPosPrevLine.y + new_item_size.y);
        window->DC.CursorPos.y -= padding;
        if (!is_closing)
            window->DC.CursorMaxPos.y -= padding;
    }

    if (layout->Type == ImGuiLayoutType_Vertical)
        layout->MaxExtent = ImMax(layout->MaxExtent, new_item_size.x);
    else
        layout->MaxExtent = ImMax(layout->MaxExtent, new_item_size.y);
...
static void AddLayoutSpring(ImGuiLayout* layout, float weight, float spacing)
{
    EndLayoutItem(layout, false);

    ImGuiLayoutItem* spring_item = PushNextLayoutItem(layout, ImGuiLayoutItemType_Spring);
    ...
    if (spring_item->SpringWeight != weight)
    {
        ...
        layout->Dirty |= ImGuiLayoutDirtyFlags_SpringWeight;
    }
    ...
    if (spring_item->SpringSpacing != spacing)
    {
        ...
        layout->Dirty |= ImGuiLayoutDirtyFlags_SpringSpacing;
    }

    if (spring_item->SpringSize > 0.0f || layout->MaxExtent > 0.0f)
    {
        ...
        if (layout->Type == ImGuiLayoutType_Horizontal)
        {
            spring_spacing = ImVec2(0.0f, style.ItemSpacing.y);
            spring_size    = ImVec2(spring_item->SpringSize, layout->MaxExtent);
        }
        else
        {
...
void ImGui::NextColumn()
{
    ...
    {
        // Columns 1+ cancel out Indent
        // FIXME: layouts, probably we should select relevant axis basing on current layout
        window->DC.ColumnsOffset = ImVec2(GetColumnOffset(window->DC.ColumnsCurrent), 0.0f) - window->DC.Indent + ImVec2(g.Style.ItemSpacing.x, 0.0f);
...
void ImGui::Columns(int columns_count, const char* id, bool border, ImGuiItemGetter items_getter, void *data)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    window->DC.ColumnsMinX = window->DC.Indent.x; // Lock our horizontal range
...
{
    // FIXME: Selecte axis according to current layout
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
