...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
    ...
    // Render
    const ImU32 grab_col = ImGui::GetColorU32(held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab,hover_envelope(id));
    if (horizontal)
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 col = GetColorU32(flags & ImGuiButtonFlags_Disabled ? ImGuiCol_TextDisabled : (hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button,
		hover_envelope(id));
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header,hover_envelope(id));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
    ...
    RenderCollapseTriangle(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + frame_bb.GetHeight()*0.5f), true,style.CollapseTriangleScale);
...
// Combo box function.
bool ImGui::Combo(const char* label, int* current_item, ImGuiItemGetter items_getter, void* data, int items_count, int height_in_items)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected || a>0.f)
    {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header,a);
...
void ImGui::BeginColumns(const char* id, int columns_count, ImGuiColumnsFlags flags,ImGuiItemGetter items_getter, void *data)
{
    ...
    window->DC.ColumnsMinX = window->DC.Indent.x - g.Style.ItemSpacing.x; // Lock our horizontal range
