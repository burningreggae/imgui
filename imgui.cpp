...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        // Framed type
        RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
        RenderCollapseTriangle(bb.Min + ImVec2(padding.x, bb.GetHeight()*0.5f), is_open, style.CollapseTriangleScale);
        if (g.LogEnabled)
        {
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    else
    {
        ...
        RenderCollapseTriangle(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f, label_size.y*0.5f), false,style.CollapseTriangleScale);
