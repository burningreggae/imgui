...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
    ...
    window->DC.CursorPos = ImVec2((float)(int)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x), (float)(int)(window->DC.CursorPos.y + lineSize.y + g.Style.ItemSpacing.y));
...
bool ImGui::_ItemHoverable(const ImRect& bb, ImGuiID id)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        for (int n = 0; n != g.Windows.Size; n++)
        {
            ...
            if ((window->Active || take) && window->HiddenFrames <= 0 && (window->Flags & (ImGuiWindowFlags_ChildWindow)) == 0 && window != window_to_render_front_most)
...
void ImGui::RenderArrow(ImVec2 p_min, ImGuiDir dir, float scale)
{
    ...
    window->DrawList->AddTriangleFilled(center + a, center + b, center + c, GetColorU32(ImGuiCol_CollapseTriangle));
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    // Title bar
    const bool is_focused = g.NavWindow && g.NavWindow == window;
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ...
    // Render
    const ImU32 grab_col = ImGui::GetColorU32(held ? ImGuiCol_ScrollbarGrabActive : ImGuiCol_ScrollbarGrab,ImGuiCol_ScrollbarGrabHovered,envelope_get(id));
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 col = GetColorU32(flags & ImGuiButtonFlags_Disabled ? ImGuiCol_TextDisabled : held ? ImGuiCol_ButtonActive : ImGuiCol_Button,ImGuiCol_ButtonHovered,envelope_get(id));
    RenderNavHighlight(bb, id);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        RenderArrow(frame_bb.Min + ImVec2(padding.x, frame_bb.GetHeight()*0.5f), is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f);
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(frame_bb.Min + ImVec2(padding.x,frame_bb.GetHeight()*0.5f), is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.70f);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(ImGuiCol_SliderGrabActive,ImGuiCol_SliderGrab,envelope_get(id)), style.GrabRounding);
...
bool ImGui::DragBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max, int decimal_precision, float power)
{
    ...
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : ImGuiCol_FrameBg,ImGuiCol_FrameBgHovered,envelope_get(id));
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, style.FrameRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive :  ImGuiCol_FrameBg,ImGuiCol_FrameBgHovered,envelope_get(id)),true, style.FrameRounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(ImGuiCol_Button,ImGuiCol_ButtonHovered,envelope_get(id)), style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
        RenderArrow(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + frame_bb.GetHeight()*0.5f), ImGuiDir_Down);
    }
...
bool ImGui::Combo(const char* label, int* current_item, ImGuiItemGetter items_getter, void* data, int items_count, int popup_max_height_in_items)
{
    ...
    if (*current_item >= 0 && *current_item < items_count)
        items_getter(data, *current_item, &preview_text,ImGuiItemGetterCommand_get_text);
...
*/
    if (held || selected || a > 0.f)
    {
        const ImU32 col = GetColorU32(held ? ImGuiCol_HeaderActive: selected ? ImGuiCol_Header:ImGuiCol_HeaderHovered,ImGuiCol_HeaderHovered,a);
        RenderFrame(bb_with_spacing.Min, bb_with_spacing.Max, col, false, 0.0f);
    }
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    else
    {
        ...
        RenderArrow(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.30f, label_size.y*0.5f), ImGuiDir_Right);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    {
		SetCursorPos(ImVec2(style.ItemSpacing.x + 2.f, sv_picker_size+style.ItemSpacing.y*3.f));
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags,ImGuiItemGetter items_getter, void *data)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    window->DC.ColumnsOffset.y = 0.0f;
    window->DC.CursorPos.x = (float)(int)(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x);
