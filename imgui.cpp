...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
    ...
    if (!window->DC.CurrentLayout || window->DC.CurrentLayout->Type == ImGuiLayoutType_Vertical)
    {
        ...
        window->DC.CursorPos = ImVec2(
            (float)(int)(window->Pos.x + window->DC.IndentX + window->DC.ColumnsOffsetX),
    ...
    else
    {
        ...
        window->DC.PrevLineSize = ImVec2(lineSize.x, window->DC.CurrentLineHeight);
    }
#endif
...
//  return (window->Active) && (!window->Hidden);
    //window is active if envelope has to be drawn
...
void ImGui::RenderArrow(ImVec2 p_min, ImGuiDir dir, float scale)
{
    ...
    g.CurrentWindow->DrawList->AddTriangleFilled(center + a, center + b, center + c, GetColorU32(ImGuiCol_CollapseTriangle));
}
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    flags |= ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoSavedSettings|ImGuiWindowFlags_ChildWindow|ImGuiWindowFlags_NoShadows;
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay)
{
    ...
    // Render
    const ImU32 col = GetColorU32(held ? ImGuiCol_SeparatorActive : ImGuiCol_Separator,g.HoveredIdTimer >= hover_visibility_delay ? ImGuiCol_SeparatorHovered:ImGuiCol_Separator,envelope_get(id));
