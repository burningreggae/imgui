...
public:
    ...
    float       TitleBarHeight() const                  { return (Flags & ImGuiWindowFlags_NoTitleBar) ? 0.0f : CalcFontSize() + GImGui->Style.TitleBarHeight; } //GImGui->Style.FramePadding.y * 2.0f; }
...
namespace ImGui
{
    ...
    IMGUI_API bool          _ItemHoverable(const ImRect& bb, ImGuiID id);
    ...
    IMGUI_API void          BeginColumns(const char* str_id, int count, ImGuiColumnsFlags flags = 0, ImGuiItemGetter items_getter= 0, void *data=0); // setup number of columns. use an identifier to distinguish multiple column sets. close with EndColumns().
